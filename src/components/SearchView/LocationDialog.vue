...
async function getFlag(country: string) {
    const unknownFlag = "data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath fill='%23ccc' d='M0 0h512v512H0z'/%3E%3Ctext fill='%23999' x='256' y='384' style='font: bold 333px sans-serif' text-anchor='middle'%3E%3F%3C/text%3E%3C/svg%3E";
...
function saveLocation() {
    session.setLocation(searchResult.value[selectionId.value].geometry.coordinates[1], searchResult.value[selectionId.value].geometry.coordinates[0], searchResult.value[selectionId.value].properties.county, searchResult.value[selectionId.value].properties.country, searchResult.value[selectionId.value].properties.label, searchResult.value[selectionId.value].properties.country_a);
}
</script>
...
<template>
    <Dialog v-if="!isMobile">
        ...
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Modifiez la localisation par défaut en effectuant une recherche ci-dessous. Cliquez sur Sauvergarder
                    une fois terminé.
                </DialogDescription>
            </DialogHeader>
            ...
            <div class="flex w-full gap-4">
                <Input id="search" type="text" v-model="searchInput"
                    placeholder="Entrez une ville, une addresse (recommandé)..." :disabled="isSearching"
                    @keyup.enter="search" />
                <Button variant="secondary" @click="search" class="w-40 flex items-center justify-center"
            ...
            <ScrollArea v-show="searchResult.length > 0 && !isSearching" class="h-[200px] w-full">
                <div class="flex cursor-pointer items-center justify-between border p-3 rounded-lg first:mt-0 mt-4"
                    v-for="(item, index) in searchResult" :key="index" @click="selectionId = index">
                    <div class="w-5 h-5 rounded-full mr-4 flex items-center justify-center"
                        :class="{ 'bg-primary': selectionId == index, 'border': selectionId != index, 'border-primary': selectionId != index }">
            ...
            <div v-show="searchResult.length == 0 && !isSearching"
                ...
                <p class="text-muted-foreground text-center">Aucun résultat.<br />Effectuez une recherche ou changez
                    celle-ci.
                </p>
            </div>
            ...
            <DialogFooter>
                <DialogClose as-child>
                    <Button type="submit" @click="saveLocation"
                        :disabled="selectionId == -1 || isSearching || searchResult.length == 0">Sauvegarder</Button>
                </DialogClose>
            </DialogFooter>
        </DialogContent>
    </Dialog>
    ...
    <Drawer v-if="isMobile">
        ...
        <DrawerContent>
            ...
            <div v-show="isSearching" class="w-full h-[200px] flex flex-col items-center justify-center px-4">
                <Skeleton class="w-full h-[50px] rounded-lg first:mt-0 mt-4" />
                <Skeleton class="w-full h-[50px] rounded-lg first:mt-0 mt-4" />
                <Skeleton class="w-full h-[50px] rounded-lg first:mt-0 mt-4" />
            </div>
            ...
            <DrawerFooter>
                <DrawerClose>
                    <Button @click="saveLocation" class="w-full mb-4"
